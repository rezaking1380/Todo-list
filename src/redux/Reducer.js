...
const Reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case TODO_EDIT:
      ...
      return editTask;
    case TODO_COMPLETE:
      const isComplete = state.tasks.map((task) =>
        task.id === action.id
          ? { ...task, compleate: (task.compleate = !task.compleate) }
          : task
      );
      const changeCategory = isComplete.map((task) =>
        task.compleate === false && task.category !== "doing"
          ? { ...task, category: (task.category = "todo") }
          : task
      );
      const completed = changeCategory.map((task) =>
        task.compleate === true
          ? { ...task, category: (task.category = "done") }
          : task
      );
      const completeTask = {
        ...state,
